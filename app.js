...
async function loadDragonModel() {
  ...
  // bunny.glb はMeshopt圧縮なし。デコーダーは未使用でも害はないため残しています。
...
function detectHandsIfNeeded(now) {
  ...
  if (handInfo.isOpen) {
    setStatus(`バニーを表示中 / ${currentHandedness === "Right" ? "右手" : currentHandedness === "Left" ? "左手" : "手"} / 手のひら開`);
  } else {
...
function updateDragon(time, delta) {
  ...
  // 龍用の浮遊・傾きゆらぎは廃止。
  // ダンスアニメーション自体が動きを持つため、二重の揺れを避けています。
  ...
  if (dragonModel) {
    dragonModel.rotation.y = DRAGON_ROTATION_Y;
  }
